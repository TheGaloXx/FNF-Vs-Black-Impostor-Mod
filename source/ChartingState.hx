...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		var notetypetext:FlxText = new FlxText(60,90,0, "Charting: " + currentNote, 16);
	...
	function addNoteUI():Void
	{
		...
		var normalnote:FlxButton = new FlxButton(10, 200, 'Normal Note', notenormal);

		var warningnote:FlxButton = new FlxButton(10, 250, 'Warning Note', notewarning);

		var dangernote:FlxButton = new FlxButton(10, 300, 'Danger Note', notedanger);

		tab_group_note.add(stepperSusLength);
		tab_group_note.add(stepperSusLengthLabel);
		tab_group_note.add(applyLength);
		tab_group_note.add(normalnote);
		tab_group_note.add(warningnote);
		tab_group_note.add(dangernote);
	...
	private function addNote(?n:Note):Void
	{
		...
		var noteSus = 0;
		var noteGod = styles[this.noteGod];

		if (n != null)
			_song.notes[curSection].sectionNotes.push([n.strumTime, n.noteData, n.sustainLength]);
		else
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, noteGod]);
